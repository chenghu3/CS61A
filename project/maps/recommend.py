...
def find_centroid(cluster):
    ...
    for restaurant in cluster:
        ...
        longtitude.append(restaurant_location(restaurant)[1])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    predictor_r2 = [find_predictor(user, reviewed, feature_fn) for feature_fn in feature_fns]
    return max(predictor_r2, key = lambda pair: pair[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            all_rating[name] = user_rating(user, name)
        else:
            all_rating[name] = predictor(restaurant)
    return all_rating
