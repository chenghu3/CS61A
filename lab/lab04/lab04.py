...
def closer_city(lat, lon, city1, city2):
    ...
    if distance(test_city, city1) > distance(test_city, city2):
...
def put_piece(board, max_rows, column, player):
    ...
    "*** YOUR CODE HERE ***"
    test_row = max_rows - 1
    while test_row >= 0 and get_piece(board, test_row, column) != '-':
        test_row -= 1
    if test_row >= 0:
        new_row = replace_elem(board[test_row], column, player)
        new_board = replace_elem(board, test_row, new_row)
        board = new_board
    return test_row, board
